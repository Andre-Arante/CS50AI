...
class Sentence():
    ...
    def known_mines(self):
        ...
        # If count is equal to set length, all cells in set are mines
    ...
    def known_safes(self):
        ...
        # If count is zero, all cells in set are safe
        if self.count == 0:
            return self.cells
        else:
            return set()
        ...
        for i in range(cell[0] - 1, cell[0] + 2):
            for j in range(cell[1] - 1, cell[1] + 2):  
                ...
                if (0 <= i < self.width) and (0 <= j < self.height):
                    additional_cells.add((i, j))
                    
        if count == 0:
            for cell in additional_cells:
                self.mark_safe(cell)
        
        New_Knowledge = Sentence(cells=additional_cells, count=count)

        # This section updates the knowledge base
        knowledge_copy = copy.deepcopy(self.knowledge)

        for K in self.knowledge:

                if K.cells == New_Knowledge.cells:
                    continue
                ...
                if K.cells.issubset(New_Knowledge.cells):
                    ...
                    # If set difference exists, create a new statement
                    if set_diff: 
                        Additional_Knowledge = Sentence(cells=set_diff, count=count_diff)
                        knowledge_copy.append(Additional_Knowledge)

                        for cell in Additional_Knowledge.known_safes():
                            self.mark_safe(cell)
                        for cell in Additional_Knowledge.known_mines():
                            self.mark_mine(cell)
